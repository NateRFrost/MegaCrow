import { e_custom_variable_type } from "@blamnetwork/blf/haloreach/v11860_10_07_24_0147_omaha_release";
...
const CUSTOM_VARIABLE_TYPE_TO_BLF = {
  ...
  Record<CustomVariableType, e_custom_variable_type>
>;
...
export const encodeCustomVariableType = (
  ...
  const mapped = (
    CUSTOM_VARIABLE_TYPE_TO_BLF as Partial<
      Record<
        string,
        (typeof CUSTOM_VARIABLE_TYPE_TO_BLF)[keyof typeof CUSTOM_VARIABLE_TYPE_TO_BLF]
      >
    >
  )[value as never];
  if (mapped === undefined) {
    throw new Error(
      `Custom variable type ${value} is not supported on this version`
    );
  }
  return mapped;
};
