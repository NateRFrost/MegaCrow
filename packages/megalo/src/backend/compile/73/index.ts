import { c_game_engine_custom_variant } from "@blamnetwork/blf/haloreach/v09730_10_04_09_1309_omaha_delta";
import { c_string_table } from "@blamnetwork/blf/haloreach/v12065_11_08_24_1738_tu1actual";

import { compileActions } from "src/backend/compile/73/actions";
import { CAPABILITES_73 } from "src/backend/compile/73/capabilities";
import { compileConditions } from "src/backend/compile/73/conditions";
import {
  applyPlayerTraitOptionOverrides,
  applyUserDefinedOptionOverrides,
  compileGameOptions,
} from "src/backend/compile/73/game_options";
import { compileGameStats } from "src/backend/compile/73/game_stats";
import { compileHudWidgets } from "src/backend/compile/73/hud_widgets";
import { compileLoadoutPalettes } from "src/backend/compile/73/loadout_palette";
import { compileMapObjects } from "src/backend/compile/73/map_object";
import { compileMetadata } from "src/backend/compile/73/metadata";
import { packCompiledVariant } from "src/backend/compile/73/pack";
import { compileRequisitions } from "src/backend/compile/73/requisitions";
import { compileTeams } from "src/backend/compile/73/teams";
import { compileTriggers } from "src/backend/compile/73/triggers";
import { compileVariableMetadata } from "src/backend/compile/73/variableMetadata";
import {
...
/** omaha_delta script string table bitstream layout. */
const SCRIPT_STRINGS = {
  maxStringCount: 112,
  maxStringLength: 0x4c_00,
  offsetBitLength: 14,
  bufferSizeBitLength: 14,
  countBitLength: 7,
} as const;
...
const DEFAULT_BUILD_NUMBER = 9730;

export class Compiler73 extends Compiler {
  ...
  private makeDefaultGametype(): c_game_engine_custom_variant {
    ...
    gametype.initialize();
    gametype.m_encoding_version = 73;
  ...
  private compile(
    ...
    compileRequisitions(ir, gametype, diagnostics);
    ...
    if (!isBaseDerived) {
      ...
      compileTriggers(ir, gametype, diagnostics);
    }

    if (gametype.m_build_number < 0) {
      gametype.m_build_number = DEFAULT_BUILD_NUMBER;
    }
    gametype.m_encoding_version = 73;
  ...
  public getCapabilities(): CompilerCapabilities {
    return CAPABILITES_73;
  }

  public getMegaloVersion(): SupportedMegaloVersion {
    return MEGALO_VERSIONS["73"];
  }
}
