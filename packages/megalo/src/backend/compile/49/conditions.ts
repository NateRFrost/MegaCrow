...
import { encodeConditionType } from "src/backend/compile/49/enums/e_condition_type";
import { encodeDisposition } from "src/backend/compile/49/enums/e_disposition";
import { encodeNumericComparison } from "src/backend/compile/49/enums/e_numeric_comparison";
import {
...
/** Alpha packs killer-type as a raw 5-bit integer. */
const encodeKillerTypeFlags = (value: PlayerDeathKillerTypeFlags): number => {
  let bits = 0;
...
const compileCondition = (condition: Condition): c_condition => {
  ...
  switch (condition.type) {
    ...
    default:
      throw new Error(
        `Condition ${condition.type} is not supported on encoding 49`
      );
  }
...
export const compileConditions = (
  ...
  gameVariant.m_game_engine.m_conditions =
    ir.gameVariant.gameEngine.conditions.map(compileCondition);
};
