import {
  ...
  s_action_object_set_minimap_icon_parameters,
  s_action_object_set_minimap_priority_parameters,
  s_action_object_set_minimap_visibility_parameters,
...
import { encodeActionType } from "src/backend/compile/49/enums/e_action_type";
import { encodeGameEngineTimerRate } from "src/backend/compile/49/enums/e_game_engine_timer_rate";
import { encodeGrenadeType } from "src/backend/compile/49/enums/e_grenade_type";
import { encodeMathOperation } from "src/backend/compile/49/enums/e_math_operation";
import { encodeMegaloSound } from "src/backend/compile/49/enums/e_megalo_sound";
import { encodeNavpointPriority } from "src/backend/compile/49/enums/e_navpoint_priority";
import { encodePlayerFilterType } from "src/backend/compile/49/enums/e_player_filter_type";
import { encodeWeaponPickupPriority } from "src/backend/compile/49/enums/e_weapon_pickup_priority";
import {
  encodeBoundaryShape,
...
} from "src/backend/compile/49/references";
import {
  BUILT_IN_LOCATION,
  type Diagnostics,
  UNKNOWN_LOCATION,
...
const compileAction = (
  ...
  switch (action.type) {
    ...
    case ActionType.navpoint_set_visible: {
      ...
      params.m_value = encodePlayerFilterType(
        action.parameters.playerFilterModifier.type
      );
    ...
    case ActionType.set_progress_bar: {
      ...
      params.m_object = encodeObjectReference(action.parameters.object);
      params.m_team_filter = encodePlayerFilterType(
        action.parameters.playerFilterModifier.type
      );
      params.m_timer_index = action.parameters.timerIndex;
    ...
    case ActionType.boundary_set_visible: {
      ...
      params.m_object = encodeObjectReference(action.parameters.object);
      const filter = action.parameters.playerFilterModifier;
      if (filter.type === PlayerFilterType.player) {
        params.m_variable = encodeCustomVariableReference(filter.visible);
      }
    ...
    case ActionType.object_set_scale: {
      ...
      params.m_object = encodeObjectReference(action.parameters.object);
      const scale = action.parameters.scale;
      params.m_scale =
        scale.kind === "float"
          ? scale.value
          : scale.value.type === CustomVariableType.Constant
            ? scale.value.immediateValue
            : 0;
    ...
    case ActionType.object_set_minimap_visibility: {
      const params = new s_action_object_set_minimap_visibility_parameters();
      params.m_object = encodeObjectReference(action.parameters.object);
      params.m_value = action.parameters.visible ? 1 : 0;
      target.m_object_set_minimap_visibility_parameters = params;
      break;
    }
    case ActionType.object_set_minimap_priority: {
      const params = new s_action_object_set_minimap_priority_parameters();
      params.m_object = encodeObjectReference(action.parameters.object);
      params.m_value = encodeNavpointPriority(action.parameters.priority);
      target.m_object_set_minimap_priority_parameters = params;
      break;
    }
    case ActionType.object_set_minimap_icon: {
      const params = new s_action_object_set_minimap_icon_parameters();
      params.m_object = encodeObjectReference(action.parameters.object);
      params.m_icon_index = action.parameters.iconIndex;
      target.m_object_set_minimap_icon_parameters = params;
      break;
    }
