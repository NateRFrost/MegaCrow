...
/**
 * Assign `getIRValue(value)` to `target[key]` only when the IR provides the option.
 */
const assign = <Target, Key extends keyof Target>(
  target: Target,
  key: Key,
  value: ValueWithLocation<number | boolean | string> | undefined
): void => {
  const resolved = getIRValue(value);
  if (resolved !== undefined) {
    target[key] = resolved as Target[Key];
  }
};
...
const assignBoolean = <Target, Key extends keyof Target>(
  target: Target,
  key: Key,
  value: ValueWithLocation<number | boolean> | undefined
): void => {
  const resolved = getIRValue(value);
  if (resolved !== undefined) {
    target[key] = Boolean(resolved) as Target[Key];
  }
};
...
const compileTeamOption = (
  ...
  const color = getIRValue(team.teamColor);
  if (color !== undefined) {
    target.m_override_color_armour = true;
    target.m_team_color_override = encodeColor(color);
  }
  const name = getIRValue(team.name);
  if (name !== undefined) {
...
const compileLoadout = (
  ...
  assign(target, "m_initial_primary_weapon_absolute_index", loadout.initialPrimaryWeaponAbsoluteIndex);
  assign(target, "m_initial_secondary_weapon_absolute_index", loadout.initialSecondaryWeaponAbsoluteIndex);
  assign(target, "m_initial_equipment_absolute_index", loadout.initialEquipmentAbsoluteIndex);
  const grenadeCount = getIRValue(loadout.initialGrenadeCountSetting);
  if (grenadeCount !== undefined) {
    target.m_initial_grenade_count_setting = encodeGrenadeCountSetting(grenadeCount);
  }
};
...
export const compileGameOptions = (
  ...
  assign(misc, "m_round_time_limit_minutes", miscellaneousOptions.roundTimeLimitMinutes);
  assign(misc, "m_round_limit", miscellaneousOptions.roundCount);
  assign(misc, "m_early_victory_win_count", miscellaneousOptions.earlyVictoryWinCount);
  assign(misc, "m_sudden_death_time", miscellaneousOptions.suddenDeathTimeLimitSeconds);
  assign(misc, "m_grace_period", miscellaneousOptions.gracePeriodTimeLimitSeconds);
  ...
  assign(respawn, "m_respawn_with_teammate", respawnOptions.respawnWithTeammate);
  ...
  assign(respawn, "m_suicide_penalty_seconds", respawnOptions.suicidePenaltySeconds);
  assign(respawn, "m_betrayal_penalty_seconds", respawnOptions.betrayalPenaltySeconds);
  assign(respawn, "m_respawn_growth_seconds", respawnOptions.respawnGrowthSeconds);
  ...
  assignBoolean(social, "friendly_fire_enabled", socialOptions.friendlyFireEnabled);
  assignBoolean(social, "betrayal_booting_enabled", socialOptions.betrayalBootingEnabled);
  assignBoolean(social, "enemy_voice_enabled", socialOptions.enemyVoiceEnabled);
  assignBoolean(social, "open_channel_voice_enabled", socialOptions.openChannelVoiceEnabled);
  assignBoolean(social, "dead_player_voice_enabled", socialOptions.deadPlayerVoiceEnabled);
  ...
  assignBoolean(map.m_flags, "shortcuts_on_map", mapOverrideOptions.shortcutsOnMap);
  assignBoolean(map.m_flags, "equipment_on_map", mapOverrideOptions.equipmentOnMap);
  ...
  assignBoolean(map.m_flags, "indestructible_vehicles", mapOverrideOptions.indestructibleVehicles);
  const weaponSet = getIRValue(mapOverrideOptions.weaponSetAbsoluteIndex);
  if (weaponSet !== undefined) {
    map.m_weapon_set_absolute_index = encodeWeaponSet(weaponSet);
  }
  const vehicleSet = getIRValue(mapOverrideOptions.vehicleSetAbsoluteIndex);
  if (vehicleSet !== undefined) {
    map.m_vehicle_set_absolute_index = encodeVehicleSet(vehicleSet);
  }
  if (mapOverrideOptions.basePlayerTraits !== undefined) {
    encodePlayerTraits(map.m_base_player_traits, mapOverrideOptions.basePlayerTraits, {
      mapOverrideSpeed: true,
    });
  }
  if (mapOverrideOptions.redPowerupTraits !== undefined) {
    encodePlayerTraits(map.m_red_powerup_traits, mapOverrideOptions.redPowerupTraits, {
      mapOverrideSpeed: true,
    });
  }
  if (mapOverrideOptions.bluePowerupTraits !== undefined) {
    encodePlayerTraits(map.m_blue_powerup_traits, mapOverrideOptions.bluePowerupTraits, {
      mapOverrideSpeed: true,
    });
  }
  if (mapOverrideOptions.yellowPowerupTraits !== undefined) {
    encodePlayerTraits(map.m_yellow_powerup_traits, mapOverrideOptions.yellowPowerupTraits, {
      mapOverrideSpeed: true,
    });
  }
  assign(map, "m_red_powerup_duration_seconds", mapOverrideOptions.redPowerupDurationSeconds);
  assign(map, "m_blue_powerup_duration_seconds", mapOverrideOptions.bluePowerupDurationSeconds);
  assign(map, "m_yellow_powerup_duration_seconds", mapOverrideOptions.yellowPowerupDurationSeconds);
  ...
  assign(teamOptions, "m_designator_switch_type", irTeamOptions.designatorSwitchType);
  ...
  assignBoolean(loadouts.m_flags, "spartan_loadouts_enabled", loadoutTraits.spartanLoadoutsEnabled);
  assignBoolean(loadouts.m_flags, "elite_loadouts_enabled", loadoutTraits.eliteLoadoutsEnabled);
  ...
  gameVariant.m_player_traits = irVariant.playerTraits.map((option) =>
    compilePlayerTraitOption(
      Number(getIRValue(option.name) ?? 0),
      Number(getIRValue(option.description) ?? 0),
      getIRValue(option.traits) as PlayerTraits
    )
  );
  ...
  if (playerRatings !== undefined) {
    ...
    assignBoolean(ratings, "m_normalize_by_max_kills", playerRatings.normalizeByMaxKills);
  ...
  assignBoolean(tu1.m_flags, "always_spillover_damage", tu1Settings.alwaysSpilloverDamage);
  assignBoolean(tu1.m_flags, "armor_lock_stickies_remain", tu1Settings.armorLockStickiesRemain);
  assignBoolean(tu1.m_flags, "attached_damage_bypass_shields", tu1Settings.attachedDamageBypassShields);
  assignBoolean(tu1.m_flags, "active_camo_override_energy_curve", tu1Settings.activeCamoOverrideEnergyCurve);
  assignBoolean(tu1.m_flags, "sword_gun_clang_kills", tu1Settings.swordGunClangKills);
  ...
  assign(tu1, "m_armor_lock_damage_drain_limit", tu1Settings.armorLockDamageDrainLimit);
  assign(tu1, "m_active_camo_energy_curve_min", tu1Settings.activeCamoEnergyCurveMin);
  assign(tu1, "m_active_camo_energy_curve_max", tu1Settings.activeCamoEnergyCurveMax);
