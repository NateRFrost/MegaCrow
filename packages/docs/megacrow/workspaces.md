...
A **workspace** points MegaCrow at a Halo Reach Editing Kit (HREK) Megalo scripts folder so the Files sidebar can browse, open, and compile your `.txt` gametypes.
...
| **Delete** | Removes the workspace from MegaCrow (does not delete files on disk) |
...
|-------|----------|-------------|
| **Name** | Yes | Display name in the workspace switcher. When you pick a scripts folder, MegaCrow may fill this from HREK `project.xml` if the name is still empty. |
...
When you browse for a scripts folder, MegaCrow tries to:
