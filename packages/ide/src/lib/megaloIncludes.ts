...
/** @deprecated Use inferWorkspaceIncludeRoot with the active workspace. */
export async function inferHrekIncludeRoot(
  fileName: string
): Promise<MegaloIncludeRoot | null> {
  const { getActiveWorkspace } = await import("./workspace");
  return inferWorkspaceIncludeRoot(fileName, getActiveWorkspace());
}
